...
public class DynamicStairs extends Plugin {
	public class dynamicStairsAction extends PluginAction {
		...
		public Object bCode; // Building code parameters, varies by location, set in setBuildingCode
		...
		public void execute() {
			...
			double maxSteps = Math.ceil(staircase.rise / bCode.minRising);
			double minSteps = Math.ceil(staircase.rise / bCode.maxRising);
			double minFlights = Math.ceil(minSteps / bCode.maxRisersPerFlight);

		}

		private boolean checkSlope(double rise, double going) {
			// test rise, going, 2R+G are within bounds
			return staircase.rising >= bCode.minRising && staircase.rising <= bCode.maxRising && 
					staircase.going >= bCode.minGoing  && staircase.going <= bCode.maxGoing && 
					2 * staircase.rising + staircase.going >= bCode.min2RplusG && 
					2 * staircase.rising + staircase.going <= bCode.max2RplusG;
		}
